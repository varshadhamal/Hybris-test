...
LOG_4J_LIB="$CORE_LIB/log4j-1.2.17.jar:$CORE_LIB/slf4j-api-1.7.13.jar:$CORE_LIB/log4j-api-2.4.1.jar:$CORE_LIB/log4j-core-2.4.1.jar:$CORE_LIB/log4j-slf4j-impl-2.4.1.jar"
GUAVA_LIB="$CORE_LIB/guava-18.0.jar"
...
java -classpath ${LIKEY_LIB}:${IO_UTILS_LIB}:${COMMONS_LANG_LIB}:${SPRING_LIB}:${LOG_4J_LIB}:${GUAVA_LIB}:${CORE_SERVER}:${CORE_CLASSES}:${BOOTSTRAP_JAR}:${BOOTSTRAP_CLASSES}:${CORE_RESOURCES} \
    -Dpcd.home=${BASE_DIR} \
