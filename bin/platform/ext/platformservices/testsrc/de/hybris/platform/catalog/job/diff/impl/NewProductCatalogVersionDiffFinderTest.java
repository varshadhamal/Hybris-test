...
/**
 * Test covering {@link NewProductCatalogVersionDiffFinder} logic.
 */
@UnitTest
public class NewProductCatalogVersionDiffFinderTest
{
	@Mock
	...
	private NewProductCatalogVersionDiffFinder finder;
	...
	public void prepare()
	{
		...
		finder = new NewProductCatalogVersionDiffFinder();
	...
	public void testPopulateDifferenceModel()
	{
		...
		Assert.assertEquals(diff.getMode().getType(), de.hybris.platform.catalog.enums.ProductDifferenceMode.PRODUCT_NEW.getType());
		Assert.assertEquals(diff.getMode().getCode(), de.hybris.platform.catalog.enums.ProductDifferenceMode.PRODUCT_NEW.getCode());
		Assert.assertEquals(diff.getSourceProduct(), null);
		Assert.assertEquals(diff.getDifferenceText(), "Product " + srcProduct.getCode() + " new in version: " + target.getVersion());
		Assert.assertEquals(diff.getTargetProduct(), srcProduct);
	...
	public void testSkipSearches()
	{
		...
		cronJob.setSearchNewProducts(Boolean.FALSE);
	...
	public void testProcessSearchesWithEmptyCategoryCount()
	{
		...
		cronJob.setSearchNewProducts(Boolean.TRUE);
	...
	public void testProcessSearchesWithFewProducts()
	{
		...
		final ArgumentMatcher<ProductCatalogVersionDifferenceModel> matcherOne = new ArgumentMatcher<ProductCatalogVersionDifferenceModel>()
		{
			...
			public boolean matches(final Object argument)
			{
				...
				if (!("Product " + productOne.getCode() + " new in version: " + target.getVersion()).equalsIgnoreCase(modelLocal
						.getDifferenceText()))
				{
					return false;
				}

				if (modelLocal.getSourceProduct() != null)
				{
					return false;
				}

				if (!modelLocal.getTargetProduct().equals(productOne))
				{
					return false;
				}

				if (!modelLocal.getMode().getCode().equals(ProductDifferenceMode.PRODUCT_NEW.getCode()))
				{
					return false;
				}
		...
		final ArgumentMatcher<ProductCatalogVersionDifferenceModel> matcherTwo = new ArgumentMatcher<ProductCatalogVersionDifferenceModel>()
		{
			...
			public boolean matches(final Object argument)
			{
				...
				final ProductCatalogVersionDifferenceModel modelLocal = (ProductCatalogVersionDifferenceModel) argument;
				if (!("Product " + productTwo.getCode() + " new in version: " + target.getVersion()).equalsIgnoreCase(modelLocal
						.getDifferenceText()))
				{
					return false;
				}
				...
				if (!modelLocal.getTargetProduct().equals(productTwo))
				{
					return false;
				}
