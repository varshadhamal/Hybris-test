...
abstract public class ItemModelTest extends ServicelayerTest
{
	...
	public void testSerializableStrategies() throws IOException, ClassNotFoundException, JaloSecurityException
	{
		...
		final ProductModel productModel1 = modelService.get(defaultProduct);
		ProductModel productModel2 = null;
		ProductModel productModel3 = null;
		ProductModel productModel3a = null;
		ProductModel productModel3b = null;
		...
		setSerializationStrategy(productModel1, new SerializationStrategyDefaultImpl());
		outputStream.writeObject(productModel1);
		outputStream.close();
		// bufferLengthLeanThenDefault[0]
		final int bufferLengthDefault = buffer.toByteArray().length;
		...
		productModel2 = (ProductModel) inputStream.readObject();
		inputStream.close();
		...
		final ItemModelConverter conv = (ItemModelConverter) ((DefaultModelService) modelService).getConverterRegistry()
				.getModelConverterByModelType(productModel1.getClass());
		final SerializationStrategyLeanImpl leanStrategy = new SerializationStrategyLeanImpl();
		setSerializationStrategy(productModel1, leanStrategy);
		outputStream.writeObject(productModel1);
		outputStream.close();
		// bufferLengthLeanThenDefault[1]
		final int bufferLengthLean = buffer.toByteArray().length;

		// Read it back in and assert it is equal to the original
		inputStream = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
		productModel3 = (ProductModel) inputStream.readObject();
		inputStream.close();
		...
		try
		{
			...
			inputStream = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
			productModel3a = (ProductModel) inputStream.readObject();
			inputStream.close();
		}
		finally
		{
		...
		try
		{
			...
			inputStream = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
			productModel3b = (ProductModel) inputStream.readObject();
			inputStream.close();
		}
		finally
		{
		...
		productModel1.setEan("foo bar");
		assertTrue(conv.isModified(productModel1));
		assertSame(leanStrategy, getSerializationStrategy(productModel1));

		buffer = new ByteArrayOutputStream();
		outputStream = new ObjectOutputStream(buffer);
		outputStream.writeObject(productModel1);
		outputStream.close();
		// bufferLengthLeanThenDefault[2]
		final int bufferLengthLeanFallingBackToDefault = buffer.toByteArray().length;

		// Read it back in and assert it is equal to the original
		inputStream = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
		productModel4 = (ProductModel) inputStream.readObject();
		inputStream.close();
		assertEquals(productModel1.getEan(), productModel4.getEan());

		assertTrue(bufferLengthDefault > bufferLengthLean);
		assertTrue(bufferLengthLean < bufferLengthLeanFallingBackToDefault);
		assertTrue(productModel1 != productModel2);
		assertEquals(productModel1, productModel2);
		assertTrue(productModel1 == productModel3);
		assertEquals(productModel1, productModel3);
		assertEquals(productModel1, productModel3a);
		assertEquals(productModel1, productModel3b);
		assertTrue(productModel1 != productModel4);
		assertEquals(productModel1, productModel4);
	}

	@Test
	public void testSerializable() throws IOException, ClassNotFoundException, JaloSecurityException
	{
		final String oldEan = (String) defaultProduct.getAttribute("ean");
		final String newEan = Long.toString(System.currentTimeMillis());

		final ProductModel productModel = modelService.get(defaultProduct);
		final UnitModel unitModel = productModel.getUnit();
		assertNotNull(unitModel);

		assertEquals(oldEan, productModel.getEan());
		productModel.setEan(newEan);
		assertEquals(newEan, productModel.getEan());
		assertEquals(oldEan, defaultProduct.getAttribute("ean"));

		final ByteArrayOutputStream buffer = new ByteArrayOutputStream();
		final ObjectOutputStream outputStream = new ObjectOutputStream(buffer);
		outputStream.writeObject(productModel);
		outputStream.close();

		final ObjectInputStream inputStream = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
		final ProductModel productModelNew = (ProductModel) inputStream.readObject();
		inputStream.close();

		assertEquals(defaultProduct, modelService.getSource(productModel));
		assertEquals(oldEan, defaultProduct.getAttribute("ean"));
		assertEquals(newEan, productModel.getEan());
		assertEquals(productModel, productModelNew);
		assertEquals(unitModel, productModelNew.getUnit());

		modelService.save(productModel);
		assertEquals(newEan, defaultProduct.getAttribute("ean"));
	}
