...
public class CartServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testGetSessionCartWhenJaloSessionUpdates()
	{
		...
		assertEquals("CartService got different session cart than JaloSession", jaloCartAutoCreated2, modelService.getSource(cart2));
	...
	public void testAppendToCart()
	{
		...
		assertEquals("Product in cart entry 1 not as expected", cartSource.getEntries().get(0).getProduct(), cartTarget
				.getEntries().get(1).getProduct());
		assertEquals("Product in cart entry 2 not as expected", cartSource.getEntries().get(1).getProduct(), cartTarget
				.getEntries().get(2).getProduct());
		...
		//check products
		assertEquals("Product in cart entry 0 not as expected", cartSource.getEntries().get(0).getProduct(), inMemoryTargetCart
				.getEntries().get(0).getProduct());
		assertEquals("Product in cart entry 1 not as expected", cartSource.getEntries().get(1).getProduct(), inMemoryTargetCart
				.getEntries().get(1).getProduct());
