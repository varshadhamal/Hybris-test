...
/**
 * Test covering {@link NewCategoryCatalogVersionDiffFinder} logic.
 */
@UnitTest
public class NewCategoryCatalogVersionDiffFinderTest
{
	private static final Logger LOG = Logger.getLogger(NewCategoryCatalogVersionDiffFinderTest.class.getName());
	...
	private NewCategoryCatalogVersionDiffFinder finder;
	...
	public void prepare()
	{
		...
		finder = Mockito.spy(new NewCategoryCatalogVersionDiffFinder());
	...
	public void testPopulateDifferenceModel()
	{
		...
		Assert.assertEquals(diff.getMode().getType(),
				de.hybris.platform.catalog.enums.CategoryDifferenceMode.CATEGORY_NEW.getType());
		Assert.assertEquals(diff.getMode().getCode(),
				de.hybris.platform.catalog.enums.CategoryDifferenceMode.CATEGORY_NEW.getCode());
		Assert.assertEquals(diff.getSourceCategory(), srcCategory);
		Assert.assertEquals(diff.getDifferenceText(), "Category " + srcCategory.getCode() + " not found in version: "
				+ cronJob.getTargetVersion().getVersion());
		Assert.assertEquals(diff.getTargetCategory(), null);
	...
	public void testSkipSearches()
	{
		...
		cronJob.setSearchNewCategories(Boolean.FALSE);
	...
	{
		final CompareCatalogVersionsCronJobModel cronJob = new CompareCatalogVersionsCronJobModel();
		cronJob.setSearchNewCategories(Boolean.TRUE);
	...
	public void testProcessSearchesWithFewCategoriesNoDifferences()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			Mockito.verify(finder).getAllCategories(cronJob.getSourceVersion(), i, 1);
		}
	}
	...
	public void testProcessSearchesWithFewCategoriesWithDifferences()
	{
		...
		final ArgumentMatcher<CategoryCatalogVersionDifferenceModel> matcher = new ArgumentMatcher<CategoryCatalogVersionDifferenceModel>()
		{
			...
			public boolean matches(final Object argument)
			{
				...
				if (model.getSourceCategory() != categoryOne)
				{
					LOG.info("Source category for a differnce model is  different  than " + categoryOne);
					return false;
				}
				if (model.getTargetCategory() != null)
				{
					LOG.info("Target category for a differnce model is not null ");
					return false;
				}

				if (!"Category category1 not found in version: target".equalsIgnoreCase(model.getDifferenceText()))
				{
					LOG.info("Differnce text for model is not equal " + "Category category1 not found in version: target");
					return false;
				}
				...
				if (model.getMode().getCode() != CategoryDifferenceMode.CATEGORY_NEW.getCode())
				{
					LOG.info("Difference mode is not a " + CategoryDifferenceMode.CATEGORY_NEW.getCode());
					return false;
				}
				return true;
			}
