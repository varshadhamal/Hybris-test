...
public class TaskServiceTest extends AbstractTaskTest
{
	...
	public void testScheduleTask() throws Exception
	{
		...
		assertEquals("testTaskRunner", taskItem.getRunnerBean());
		assertNotNull(taskItem.getExecutionDate());
		assertNull(taskItem.getNodeId());
		assertNull(taskItem.getExpirationDate());
		assertEquals(-1, taskItem.getRunningOnClusterNodeAsPrimitive());
		taskService.getEngine().start();

		// due to missing accuracy in database (adjusted) execution date may be same as (adjusted) now
		final Date now = new Date();
		assertTrue(taskItem.getExecutionDate().equals(now) || taskItem.getExecutionDate().before(now));

		final TaskRunnerResult res = waitForTask(testTaskRunner, task, 20, "Task didn't run");
		assertTrue(" Task did not finish", res.didRun());

		Thread.sleep((long) (1000 * timeFactor));

		assertFalse(taskItem.isAlive());
	}
