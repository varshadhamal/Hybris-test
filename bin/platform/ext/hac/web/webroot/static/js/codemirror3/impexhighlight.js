...
//    //Commentary
	...
	if(words[cur] === 'impex_cmd'){
		...
		if(!stream.eol()){ state.tokens.unshift(headerType); }
		return words[cur];
	}
	//Data Declaration.
	else{
  ...
  var headerType = function(stream, state){
	  var ch = stream.next();
	  if(stream.eol()){ eol(state); }
	  if(!/[a-zA-Z0-9_-]/.test(ch)){ return null; }
	  curr_state = impexStateEnum.header_type;
	  stream.eatWhile(/[a-zA-Z0-9_-]/);
	  var cur = stream.current();
	  if(!types.hasOwnProperty(cur) && !stream.eol()){ getTypeAttributes(cur); }
	  state.tokens.shift();
	  if(!stream.eol()){ 
  ...
  function header(type){
	  return function(stream, state){
		  ...
		  if(ch === dataSeparator){
			  stream.eatSpace();
			  if(stream.eol()){ state.tokens.shift(); }
			  else{
		  ...
		  if(ch === '"' || ch === "'"){
			  ...
			  if(stream.eol()){
				  ...
				  stream.next();
				  if(stream.eol()){ eol(state); }
				  else{ current_state = impexStateEnum.data; }
				  return 'impex_string';
			  }
		  }
  ...
  return {
	  startState: function() {return {tokens:[], state: ""};},
