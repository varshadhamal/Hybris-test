 ...
 */
package com.hybris.cartridge.cockpits.setup;
...
import com.hybris.cartridge.cockpits.constants.CartridgeCockpitsConstants;
 ...
 */
@SystemSetup(extension = CartridgeCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(CartridgeCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(CartridgeCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						CartridgeCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/cartridgecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
